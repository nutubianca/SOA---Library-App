...
async function apply() {
  for (let attempt = 1; attempt <= 10; attempt++) {
    try {
      ...
      await client.end();
      console.log('UserAPI: migrations applied');
      return;
    } catch (err) {
      console.log(`UserAPI: migration attempt ${attempt} failed: ${err.message}`);
      await new Promise(r => setTimeout(r, 2000));
    }
  }
  throw new Error('UserAPI: migrations failed after retries');
}
