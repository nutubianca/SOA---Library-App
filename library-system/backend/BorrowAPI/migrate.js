...
async function apply() {
  for (let attempt = 1; attempt <= 10; attempt++) {
    try {
      ...
      await client.end();
      console.log('BorrowAPI: migrations applied');
      return;
    } catch (err) {
      console.log(`BorrowAPI: migration attempt ${attempt} failed: ${err.message}`);
      await new Promise(r => setTimeout(r, 2000));
    }
  }
  throw new Error('BorrowAPI: migrations failed after retries');
}
